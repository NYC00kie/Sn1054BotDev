...
module.exports = {
	...
	async execute(interaction) {
		...
		try {
			const docs = await ChannelModel.findOne({ ownerid: targetUser.id }).exec();
			if (!docs) {
			...
			await interaction.reply({ embeds: [chstatsEmbed] });
		} catch (err) {
			console.error(err);
			await interaction.reply({ content: 'Fehler beim Abrufen der Kanal-Statistiken.', ephemeral: true });
		}
	},
};
