...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('pwtop')
		.setDescription('Zeigt dir die Top10 der Leute mit dem meisten Prestiegewert an.'),
	async execute(interaction) {
		try {
			const docs = await Sale.find().exec();
			const topDocs = docs.sort((a, b) => b.Prestiege - a.Prestiege).slice(0, 10);
			...
			topDocs.forEach((doc, i) => {
				description += `${i + 1}. ${doc.Nickname || doc.Name} mit **${doc.Prestiege}** Prestiegewert\n`;
			});
